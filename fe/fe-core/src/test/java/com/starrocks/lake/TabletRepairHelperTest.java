...
public class TabletRepairHelperTest {
    ...
    public void testRepair() {
        ...
        AdminRepairTableStmt stmt = new AdminRepairTableStmt(
                new TableRef(new TableName("db", "table"), null,
                        new PartitionNames(false, Lists.newArrayList("p1")),
                        NodePosition.ZERO),
                Maps.newHashMap(),
                NodePosition.ZERO);
    ...
    public void testGetTabletStatus() throws Exception {
        ...
        List<List<String>> result = TabletRepairHelper.getTabletStatus(
                db, table, Lists.newArrayList("p1"), null, null, 2, WarehouseManager.DEFAULT_WAREHOUSE_ID);
        ...
        result = TabletRepairHelper.getTabletStatus(db, table, Lists.newArrayList("p1"), LakeTabletStatus.NORMAL, BinaryType.EQ,
        ...
        result = TabletRepairHelper.getTabletStatus(db, table, Lists.newArrayList("p1"), LakeTabletStatus.MISSING_DATA,
                BinaryType.EQ, 5, WarehouseManager.DEFAULT_WAREHOUSE_ID);
        Assertions.assertEquals(1, result.size());
        ...
        result = TabletRepairHelper.getTabletStatus(db, table, Lists.newArrayList("p1"), LakeTabletStatus.NORMAL, BinaryType.NE,
                5, WarehouseManager.DEFAULT_WAREHOUSE_ID);
        Assertions.assertEquals(1, result.size());
        Assertions.assertEquals("12", result.get(0).get(0));
    }
