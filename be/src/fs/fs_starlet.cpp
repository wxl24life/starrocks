...
static Cache* get_shard_fs_cache(bool use_raw_path) {
...
std::shared_ptr<FileSystem> new_fs_starlet(int64_t shard_id, bool use_raw_path) {
    ...
    Cache* cache = get_shard_fs_cache(use_raw_path);
    ...
    //
    // SyncPoint allows tests to inject mock filesystem result without calling real g_worker.
    // The sentinel kUnimplemented status means "not overridden by test", so we proceed with
    // the real g_worker->get_shard_filesystem() call.
    ...
    if (fs_st.status().code() == absl::StatusCode::kUnimplemented) {
