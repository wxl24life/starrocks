...
protected:
    ...
    virtual Status _trigger_next_scan(RuntimeState* state, int chunk_source_index);
...
protected:
    ...
    // Submit a scan task to the scan executor with retry logic.
    // Returns OK if submitted (or queued for retry), RuntimeError after max retries.
    Status _submit_scan_task(workgroup::ScanTask task, int chunk_source_index);

    // Accessors for subclasses that override _trigger_next_scan.
    workgroup::ScanExecutor* scan_executor() const { return _scan_executor; }
    std::weak_ptr<QueryContext> query_ctx() const { return _query_ctx; }
    workgroup::WorkGroupPtr workgroup() const { return _workgroup; }
    RuntimeProfile::HighWaterMarkCounter* peak_scan_task_queue_size_counter() const {
        return _peak_scan_task_queue_size_counter;
    }
