...
void LakeDataSource::init_counter(RuntimeState* state) {
    ...
    _gin_index_init_timer = ADD_CHILD_TIMER(_runtime_profile, "GinIndexInit", gin_filter_name);
...
void LakeDataSource::update_counter(RuntimeState* state) {
    ...
    COUNTER_UPDATE(_gin_index_init_timer, _reader->stats().gin_index_init_ns);
