...
TEST_P(StarletFileSystemTest, test_new_fs_starlet_with_shard_id) {
    // Test creating filesystem with specific shard ID
    const int64_t test_shard_id = 20086;
    
    // Create a new shard for testing
    staros::starlet::ShardInfo shard_info;
    shard_info.id = test_shard_id;
    auto fs_info = shard_info.path_info.mutable_fs_info();
    fs_info->set_fs_type(staros::FileStoreType::S3);
    auto s3_fs_info = fs_info->mutable_s3_fs_info();
    s3_fs_info->set_bucket(config::object_storage_bucket);
    ...
    // Test basic file operations with the new filesystem
    auto test_uri = build_starlet_uri(test_shard_id, "/test_file");
    ASSIGN_OR_ABORT(auto wf, fs->new_writable_file(test_uri));
    EXPECT_OK(wf->append("test content"));
    EXPECT_OK(wf->close());
    
    // Verify file exists
    EXPECT_OK(fs->path_exists(test_uri));
    
    // Test reading the file
    ASSIGN_OR_ABORT(auto rf, fs->new_random_access_file(test_uri));
    char buf[1024];
    ASSIGN_OR_ABORT(auto nr, rf->read_at(0, buf, sizeof(buf)));
    EXPECT_EQ("test content", std::string_view(buf, nr));
...
TEST_P(StarletFileSystemTest, test_new_fs_starlet_cache) {
    // Test that the cache mechanism works correctly
    const int64_t test_shard_id = 30086;
    ...
    // Create filesystem second time (should return cached instance)
    auto fs2 = new_fs_starlet(test_shard_id);
    ASSERT_NE(fs2, nullptr);
    
    // Both should be the same instance (cached)
    EXPECT_EQ(fs1.get(), fs2.get());
    
    // Test that both filesystems work correctly
    auto test_uri = build_starlet_uri(test_shard_id, "/cache_test");
    ASSIGN_OR_ABORT(auto wf, fs1->new_writable_file(test_uri));
    EXPECT_OK(wf->append("cached content"));
    EXPECT_OK(wf->close());
    
    // Verify file exists using both filesystem instances
    EXPECT_OK(fs1->path_exists(test_uri));
    EXPECT_OK(fs2->path_exists(test_uri));
...
TEST_P(StarletFileSystemTest, test_new_fs_starlet_invalid_shard) {
    // Test creating filesystem with invalid shard ID
    const int64_t invalid_shard_id = 99999;
    
    // Should return nullptr for invalid shard ID
    auto fs = new_fs_starlet(invalid_shard_id);
    EXPECT_EQ(fs, nullptr);
}

TEST_P(StarletFileSystemTest, test_starlet_filesystem_constructor) {
    // Test the new constructor that accepts shard_fs parameter
    const int64_t test_shard_id = 40086;
    ...
    auto fs = std::make_unique<StarletFileSystem>(shard_fs);
    ASSERT_NE(fs, nullptr);
    
    // Test basic operations
    auto test_uri = build_starlet_uri(test_shard_id, "/constructor_test");
    ASSIGN_OR_ABORT(auto wf, fs->new_writable_file(test_uri));
    EXPECT_OK(wf->append("constructor test"));
    EXPECT_OK(wf->close());
...
TEST_P(StarletFileSystemTest, test_new_fs_starlet_multiple_shards) {
    // Test creating filesystems for multiple different shards
    const std::vector<int64_t> shard_ids = {50086, 50087, 50088};
    ...
    for (auto shard_id : shard_ids) {
        auto fs = new_fs_starlet(shard_id);
        ASSERT_NE(fs, nullptr);
        filesystems.push_back(fs);
    }
    
    // Test that each filesystem works independently
    for (size_t i = 0; i < shard_ids.size(); ++i) {
        auto test_uri = build_starlet_uri(shard_ids[i], "/multi_test");
        ASSIGN_OR_ABORT(auto wf, filesystems[i]->new_writable_file(test_uri));
        EXPECT_OK(wf->append(fmt::format("content for shard {}", shard_ids[i])));
        EXPECT_OK(wf->close());
        
        // Verify file exists
        EXPECT_OK(filesystems[i]->path_exists(test_uri));
