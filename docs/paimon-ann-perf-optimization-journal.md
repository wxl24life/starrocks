...
| 版本       | 日期                   | 变更摘要                                                                                                                                                                                             |
| -------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **v1.5** | **2026-06-10 09:25** | **§5.9.17 R2.E PoC A — InternalSqlExecutor 三段拆分 + bypass 理论上限上调到 200 QPS**。在 `InternalSqlExecutor.runDQL` 加 parse/plan/execStmt 三个 `Tracers.watchScope`（本地 build 部署 r2e jar）。c=50 contention 抓 5 sample 实测（排除 outlier Q3=454ms）均值：**internalSqlExec = 344ms / parse = 1ms (0.3%) / plan = 41ms (12%) / execStmt = 278ms (81%)**。**关键洞察**：parse+plan = 42ms 在 bypass 路径下**完全消失**（直接 thrift 不走 SQL 编译）。execStmt = 278ms 内部待拆，其中 BE 真正 ANN compute ≈ 7.5ms（来自历史 QueryCumulativeScanTime），剩余 ~270ms 是 Coordinator 编排 + FE↔BE thrift fragment dispatch + result collection。**bypass 理论收益重新估算**（c=50, 5 sample 实测）：(a) 乐观 execStmt → 30ms → 单 query wall 241ms → **QPS 207**（达标 200 ✅）；(b) 保守 execStmt → 60ms → wall 271ms → **QPS 185**；(c) 悲观 execStmt → 100ms → wall 311ms → **QPS 160**。**c=100 投影**：当前 wall 990ms (绝大部分排队等 Coordinator)，bypass 后排队消失，理论上限 250-300 QPS。**决策**：先做 R2.F（C 阶段）—— 给 `StmtExecutor.executeStmtWithExecPlan` 加 4 个 watchScope 拆分 createCoord/dispatch/**waitResult**/postProc，waitResult 实测值是 bypass 是否真能 ≥ 200 QPS 的核心证据。R2.F 计划写在 `~/workspace/agents/plans/r2e-execstmt-decomposition.md`：单文件改动 + 5 分钟 build/deploy + c=50 60s 回归 + 5 个 sample profile，<1h 拿到决策矩阵。 |
| **v1.4** | **2026-06-10 09:00** | **§5.9.16 R2 autonomous extension — FE 侧已榨干，瓶颈完全转入 BE inner SQL exec**。新分支 `xiaolong/perf-r2-snapshot-cache`（commit `a8f70b9a2e2`，已 push ali-origin）。**R2.A snapshot pin via `FileStoreTable.copy(scan.snapshot-id=…)` 失败回退**：copy(Map) 触发 `tryTimeTravel` → 2 次额外 REST（snapshot+schema），c=50 QPS 87→68-79；`copyWithoutTimeTravel` 更糟 (54 QPS)，已 revert。**R2.D 跨 query snapshot cache 落地**（process-wide ConcurrentMap，opt-in via `Config.enable_paimon_snapshot_id_cache`，TTL 默认 5s，default OFF）：cache_on c=50 = 85.98 QPS（vs cache_off 86.17 QPS），**净增益 ≈ 0**。R1 之后 `latestSnapshot()` REST 已被 token+role cache 推到 20-30ms 微秒级，进一步 cache 无可榨。contention profile 三连验证：cache hit 时 `resolveSnapshotId=0ms` 生效，但 `evaluate.internalSqlExec=324-367ms` 是绝对 dominator，占 ExecPlanBuild 388-401ms 的 83%+。c=100 QPS 99.51-100.22，BE load avg 17/32 = 54% 仍 wait-bound 未 CPU 饱和（之前推论 BE wait on ProtobufRpcProxy.doWaitCallback 得到 c=100 复现）。**结论：≥200 QPS 不可由 FE 改动达成**；唯一路径是 BE-side eliminate inner SQL pipeline（直接 BE thrift RPC primitive 给 PaimonGlobalIndexEvaluator）或横向扩 BE。R2 lever 之所以保留：production opt-in OFF 不影响 baseline，对运维提供未来可调谐空间（高 QPS 容忍 5s 数据可见度场景）。 |
